...
void proceed_Euler(custom_math::vector_3& pos, custom_math::vector_3& vel, const long double G, const long double dt)
{
	...
	//cout << (photon_wavelength - 1) / (wl_inf - 1) << endl << endl;

	{
		const long double wl_inf = wl_emit / sqrt(1 - Rs / r_emit);
		const long double r_inf = distance;// r_emit * 10000 * speed_of_light;
		const long double wl_sortof_int = wl_emit * (sqrt(1.0 - Rs / r_inf) / sqrt(1.0 - Rs / r_emit));

		photon_wavelength += (wl_sortof_int - photon_wavelength);
	}


	cout << (photon_wavelength) << " " << (wl_inf) << endl << endl;
	...
	if (1)//1)//vel.length() != speed_of_light)
	{
		vel.normalize();
		vel *= speed_of_light;
	}
...
void idle_func(void)
{
	...
	// do not change the dt value from 1.0
	proceed_Euler(photon_pos, photon_vel, grav_constant, 1.0);

	positions.push_back(photon_pos);

	if (1)//frame_num % 6000000 == 0)
	{
...
void motion_func(int x, int y)
{
	...
	{
		main_camera.u -= static_cast<long double>(mouse_delta_y) * u_spacer;
		main_camera.v += static_cast<long double>(mouse_delta_x) * v_spacer;
	}
	else if (true == rmb_down && (0 != mouse_delta_y))
	{
		main_camera.w -= static_cast<long double>(mouse_delta_y) * w_spacer;
